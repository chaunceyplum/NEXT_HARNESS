 ...
 *
 *  - Terminal (?sync=1 path, or a Step-Functions run finalize has already
 *    closed out): getExecution()'s `result` column already has the full
 *    BuildResponse — returned as-is.
 *  - Still in flight (Step Functions run: PENDING/RUNNING/MAX_STEPS-not-
 *    yet-finalized): `result` is still null, so this derives a live
 *    BuildResponse from the run's message history (deriveTraceFromMessages)
 *    so the UI shows progress instead of nothing. Polling this route is
 *    what drives the run detail page.
 */
...
export async function GET(
  ...
  try {
    ...
    if (record.result || record.status === 'failed') {
      // 'failed' is the ?sync=1 path's terminal status (see
      // app/api/build/route.ts) — it never populates `result` but already
      // carries `error`, so there's nothing to derive.
      return Response.json(record, { status: 200 });
    }

    // No result yet — either the ?sync=1 path's request genuinely hasn't
