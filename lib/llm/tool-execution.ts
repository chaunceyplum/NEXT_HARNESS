 ...
 * (exec-tools, and the ?sync=1 in-process path) pull in this module's
 ...
 * exec-tools and lib/llm/agent.ts (the ?sync=1 path).
 */
 ...
 * independent of the AI SDK tool wrapper below. Shared by the ?sync=1
 * in-process agent loop (via buildAiTools) and the exec-tools Lambda (aws/
 * lambdas/exec-tools), so both retry identically instead of the Lambda
