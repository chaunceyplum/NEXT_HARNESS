...
const launchModule: CapabilityModule = {
  ...
  build: (config) => {
    ...
    steps.push({
      ...
      category: 'launch',
      critical: false,
      args: { name: `${slug(domain)}_${vertical}`, platform: 'web', domains: [domain] },
    });
...
const ajoOffersModule: CapabilityModule = {
  ...
  build: (config) => {
    ...
    return placements.map((placement, idx) => ({
      ...
      critical: false,
      args: {
        name: `${vertical}_${slug(placement)}_offer`,
 ...
 */
export function planUseCase(config: SolutionConfig): UseCasePlan {
  const useCase = classifyUseCase(config);
  const ctx: ModuleContext = { aepSegmentStepIds: [] };
...
export function buildStepPlan(config: SolutionConfig): PlannedStep[] {
  return planUseCase(config).steps;
}
