 ...
 * the ?sync=1 in-process behavior: buildAiTools' execute functions run one
 * at a time within a generateText step, not concurrently).
 *
 ...
 * bit-for-bit the same code path the ?sync=1 agent uses, not a
...
export const handler = async (env: LoopEnvelope): Promise<LoopEnvelope> => {
  ...
  for (const call of calls) {
    try {
      ...
      // run the same way the ?sync=1 in-process agent did, so the UI can
      // offer to switch to the MSB status-polling view.
