 ...
 *
 * Ports the setup half of lib/llm/agent.ts's runAgent(): fetch the MCP tool
 * catalog, shortlist relevant tools (tool RAG), union with the always-on
 * grounding tools, add the full-build tool only if explicitly allowed
 * (invariant: never offered otherwise) — then persist the selected tool set
 * + initial message so every later step rebuilds the exact same tool set
